...
## Scapy version
```bash
sudo ip netns exec ns-atk python3 /home/reddy/codex-workspace/netlab/attacks/04-syn-flood/flood.py \
  --target-ip 10.0.0.10 --target-port 80 --count 5000 --pps 800
```

## Capture
```bash
sudo ip netns exec ns-srv tcpdump -ni veth-srv 'tcp[tcpflags] & tcp-syn != 0' -w /home/reddy/codex-workspace/netlab/captures/04-syn-flood.pcap
```
